...
function FieldWrapper({
    ...
    if (layout === 'full-width') {
        return (
            <div className={cn('flex flex-col gap-3 w-full p-4', className)} id={element.hook_key}>
                {hasTitle && (
    ...
    return (
        <div className="flex flex-col gap-1 w-full">
            <div className="flex items-center gap-2">
                {element.image_url && (
                    <img src={element.image_url} alt="" className="w-5 h-5 object-contain" />
                )}
    ...
    return (
        <FieldWrapper element={element}>
            <div className="flex items-center gap-1 sm:max-w-56">
                {element.prefix && (
                    <span className="text-sm text-muted-foreground shrink-0">{element.prefix}</span>
                )}
                <Input
                    type="number"
                    value={String(element.value ?? element.default ?? '')}
                ...
                />
                {element.postfix && (
                    <span className="text-sm text-muted-foreground shrink-0">{element.postfix}</span>
                )}
            </div>
        </FieldWrapper>
    );
        ...
        <FieldWrapper element={element}>
            ...
            >
                <SelectTrigger className="sm:max-w-56">
                    <SelectValue placeholder={element.placeholder ? String(element.placeholder) : 'Select...'} />
                </SelectTrigger>
                <SelectContent>
...
export function LabelField({ element }: FieldComponentProps) {
    return (
        <div className="p-4 flex justify-between gap-4 items-center" id={element.hook_key}>
            <FieldLabel element={element} />
            {element.doc_link && (
                <a
...
export function HtmlField({ element }: FieldComponentProps) {
    return (
        <div
            className={cn('w-full p-4', element.css_class)}
            id={element.hook_key}
        >
...
export function FallbackField({ element }: FieldComponentProps) {
    return (
        <div className="p-4 text-sm text-muted-foreground italic" id={element.hook_key}>
            Unsupported field type: <code className="text-xs bg-muted px-1 py-0.5 rounded">{element.variant}</code>
            {element.title && <span> — {element.title}</span>}
        </div>
    );
}

export { FieldWrapper, FieldLabel };
